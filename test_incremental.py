...
class increte(Callback):
    ...
    def on_batch_end(self, net, batch=None, training=None, **kwargs):
        #Loads the new weights onto the 
...
if __name__=='__main__':
    ...
    model=incrementer(old_model=large_model,new_model=small_model)
    ...
    classifier = skorch.NeuralNetClassifier(
            ...
            batch_size=8,
            device='cuda',
            #callbacks=[cp,cb,skorch.callbacks.ProgressBar(),inc],#Try to implement accuracy and f1 score callables here
            warm_start=True,
            )
    
    classifier.initialize()
    classifier.load_params(f_params=f'model/{model_name}best_param.pkl')
    classifier.module_ = classifier.module_.new_model
    ...
    f1=[]
    auc=[]
    test_loader1=DataLoader(test_set1,batch_size=8,shuffle=False)
    for i in range(iterations):
        print(i)
        probs=[]
        actual_labels=[]
        for test_features, actual_lb in iter(test_loader1):
    ...
    f1=[]
    auc=[]
    test_loader2=DataLoader(test_set2,batch_size=8,shuffle=False)
    for i in range(iterations):
        print(i)
        probs=[]
        actual_labels=[]
        for test_features, actual_lb in iter(test_loader2):
            prob=classifier.predict_proba(test_features)
            actual_lb=np.array(actual_lb)
            probs.append(prob)
            actual_labels.append(actual_lb)
